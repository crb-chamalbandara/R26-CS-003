"""
Build the submission-ready Word (.docx) twin of C3_ICAC2026.tex.
...
OUT = PAPER_DIR / "C3_ICAC2026.docx"
...
# ─────────────────────────────────────────────────────────────────────────
# Document assembly. All prose, tables and references live in docx_content
# so this file stays pure formatting.
# ─────────────────────────────────────────────────────────────────────────
import docx_content as C
...
def main():
    ...
    add_heading(doc, "Related Work", 1, "II.")
    for para in C.RELATED:
        add_body(doc, para)

    add_heading(doc, "System Design", 1, "III.")
    ...
    add_heading(doc, "Dataset and Two Validity Defects", 1, "IV.")
    add_body(doc, C.DATASET_INTRO)
    add_heading(doc, "Pseudo-replication", 2, "A.")
    add_body(doc, C.PSEUDOREP)
    add_heading(doc, "An out-of-scope concept", 2, "B.")
    ...
    add_heading(doc, "Scope criterion and cap", 2, "C.")
    for para in C.SCOPE:
        add_body(doc, para)

    add_heading(doc, "Model", 1, "V.")
    for para in C.MODEL:
        add_body(doc, para)

    add_heading(doc, "Evaluation", 1, "VI.")
    add_figure(doc, FIG / "fig2_headline_results.png", C.FIG2_CAPTION, width_in=3.35)
    add_heading(doc, "Protocol", 2, "A.")
    for para in C.PROTOCOL:
        add_body(doc, para)
    add_heading(doc, "Headline results", 2, "B.")
    ...
    add_heading(doc, "Why XGBoost: a five-classifier comparison", 2, "C.")
    ...
    add_heading(doc, "Fusion weight and a structural limit", 2, "D.")
    for para in C.FUSION:
        add_body(doc, para)

    add_heading(doc, "Limitations", 1, "VII.")
    add_bullets(doc, C.LIMITATIONS)

    add_heading(doc, "Conclusion", 1, "VIII.")
